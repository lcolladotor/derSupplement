...
## Identify BAM files
files <- dir('/dcl01/lieber/ajaffe/derRuns/derSupplement/simulation/hisat', pattern = paste0('[0-9]R', opt$replicate, '\\.bam$'), full.names = TRUE)
names(files) <- gsub('\\.bam', '', dir('/dcl01/lieber/ajaffe/derRuns/derSupplement/simulation/hisat', pattern = paste0('[0-9]R', opt$replicate, '\\.bam$')))


message(paste(Sys.time(), 'running featureCounts'))
featCounts <- featureCounts(files = files, annot.ext = '/dcl01/lieber/ajaffe/derRuns/derSupplement/simulation/gtf/chr17-incomplete.gtf', isGTFAnnotationFile = TRUE, useMetaFeatures = FALSE, isPairedEnd = TRUE, nthreads = 4, allowMultiOverlap = TRUE)
