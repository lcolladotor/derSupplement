...
Here we evaluate the performance of the different pipelines we used to analyze the simulated data. Evaluating the performance is tricky and depends on how you define the reference set. At this moment we are considering six options detailed later.
...
exons_one <- exons[countOverlaps(exons, trans) == 1]
...
## At the disjoint exon segment level, the percent DE is lower but
...
table(countOverlaps(exons_disjoin_one) - 1)
```

Note how the overlap between the different units of the same reference set is considerable for the first three sets.
...
* Disjoint exon segment level: `r length(exons_disjoin)`
* Exon level subsetted to those that overlap only 1 transcript: `r length(exons_one)`
* Disjoint exon segment level (overlapping only 1 transcript): `r length(exons_disjoin_one)`
...
## Evaluate at the disjoint exon segments level (overlapping only 1 transcript)
## DE if any of the transcripts it overlaps is DE
tables_exons_d_one <- mapply(count_comp, stats_GR, replicates, types,
    SIMPLIFY = FALSE, MoreArgs = list(reference = exons_disjoin_one, cut = 0.05))
empirical_exons_d_one <- emp(tables_exons_d_one)
index_exons_d_one <- mapply(index_comp, stats_GR, replicates, types,
    SIMPLIFY = FALSE, MoreArgs = list(reference = exons_disjoin_one, cut = 0.05))
case_result_exons_d_one <- mapply(case_result, index_exons_d_one, replicates,
    SIMPLIFY = FALSE, MoreArgs = list(reference = exons_disjoin_one))
```
...
## Disjoint exon segments level (overlapping only 1 transcript)
kable(emp_sum(empirical_exons_d_one), format = 'html')
```
...
The sets where we consider exons or exon segments overlapping only 1 transcript show a clear difference in power for the methods that rely on annotation between the analyses that used the complete annotation and those that used the incomplete one. The `derfinder` analyses do have higher FPR and FDR than other pipelines, but achieve nearly the same power as the other methods when the annotation is complete.
...
## Disjoint exons level (overlapping only 1 transcript)
kable(empirical_exons_d_one, format = 'html', row.names = TRUE)
```
...
When using the exon level reference set (third table), only about a tenth [(`r case_result_exons[['regionMatrix.1.edgeR']][2, 1]` + `r case_result_exons[['regionMatrix.1.edgeR']][2, 2]`) / `r case_result_exons[['regionMatrix.1.edgeR']][2, 5]` = `r round(100 * (case_result_exons[['regionMatrix.1.edgeR']][2, 1] + case_result_exons[['regionMatrix.1.edgeR']][2, 2]) / case_result_exons[['regionMatrix.1.edgeR']][2, 5], 2)`%] of the `noneDE` cases are incorrectly labeled. The numbers are similar in the disjoint exon segments reference set (fourth table): [(`r case_result_exons_d[['regionMatrix.1.edgeR']][2, 1]` + `r case_result_exons_d[['regionMatrix.1.edgeR']][2, 2]`) / `r case_result_exons_d[['regionMatrix.1.edgeR']][2, 5]` = `r round(100 * (case_result_exons_d[['regionMatrix.1.edgeR']][2, 1] + case_result_exons_d[['regionMatrix.1.edgeR']][2, 2]) / case_result_exons_d[['regionMatrix.1.edgeR']][2, 5], 2)`%]
...
case_result_exons_d_one[['regionMatrix.1.edgeR']]
```
...
round(case_result_exons_d_one[['regionMatrix.1.edgeR']] /
    case_result_exons_d_one[['regionMatrix.1.edgeR']][6, 5] * 100, 2)
```
...
The percent of false negatives is further decreased on the exon and disjoint exon segment sets subsetted to those that overlap only 1 transcript. That is, the reference units where there is no ambiguity in terms of the the assignment of the DE status.

```{r 'saveResults', echo = FALSE}
## Calculate summaries
empirical_trans_sum <- emp_sum(empirical_trans)
empirical_trans_c_sum <- emp_sum(empirical_trans_c)
empirical_exons_sum <- emp_sum(empirical_exons)
empirical_exons_d_sum <- emp_sum(empirical_exons_d)
empirical_exons_one_sum <- emp_sum(empirical_exons_one)
empirical_exons_d_one_sum <- emp_sum(empirical_exons_d_one)

## Save results
save(tables_trans, empirical_trans, empirical_trans_sum, case_result_trans, file = 'results-trans.Rdata')
save(tables_trans_c, empirical_trans_c, empirical_trans_c_sum, case_result_trans_c, file = 'results-trans_c.Rdata')
save(tables_exons, empirical_exons, empirical_exons_sum, case_result_exons, file = 'results-exons.Rdata')
save(tables_exons_d, empirical_exons_d, empirical_exons_d_sum, case_result_exons_d, file = 'results-exons_d.Rdata')
save(tables_exons_one, empirical_exons_one, empirical_exons_one_sum, case_result_exons_one, file = 'results-exons_one.Rdata')
save(tables_exons_d_one, empirical_exons_d_one, empirical_exons_d_one_sum, case_result_exons_d_one, file = 'results-exons_d_one.Rdata')
```
